...
from backward_ica.online_smoothing import ELBOGradsReparam
...
def online_elbo_and_grad_func(key, obs_seq):
    stats = online_elbo_and_grads.batch_compute(key, obs_seq, p.format_params(theta), phi)
    elbo, grad_elbo = stats['Omega'], stats['jac_Omega']
    return elbo / len(obs_seq), jax.flatten_util.ravel_pytree(grad_elbo)[0]
...
plt.savefig('elbo_errors')

grad_elbo_errors.plot(kind='box')
plt.savefig('grad_elbo_errors')
