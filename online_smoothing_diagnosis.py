...
from backward_ica.svi import GeneralBackwardELBO, LinearGaussianELBO, OnlineGeneralBackwardELBO, OnlineGeneralBackwardELBOSpecialInit, OnlineGeneralBackwardELBOV2
...
def main(args, args_p, args_q):
  ...
  if args.evolution_wrt_seq_length != -1:
      ...
      plt.savefig(os.path.join(args.save_dir, 'errors_wrt_length_violinplot'))
      plt.close()
  ...
  if args.evolution_wrt_num_samples != -1:
      ...
      plt.savefig(os.path.join(args.save_dir, 'errors_wrt_num_samples_violinplot'))
      plt.close()
      ...
      plt.savefig(os.path.join(args.save_dir, 'errors_wrt_state_dim_violinplot'))
      plt.close()

  elif args.compare_online_versions:
      key, *subkeys = jax.random.split(key, 3)

      p, q, theta, phi, sampler, true_elbo = get_sampler_and_true_elbo(key, args, args_p, args_q)
      ...
      errors_online_v1 = {k:v.tolist() for k,v in enumerate(errors_online_v1)}
      errors_online_v2 = {k:v.tolist() for k,v in enumerate(errors_online_v2)}
      ...
      plt.ylabel('$\hat{\mathcal{L}}(\\theta, \\lambda) - \mathcal{L}(\\theta, \\lambda)$')
   ...
   errors = pd.DataFrame((offline_values - true_elbo_values).tolist(), columns=['Errors'])
