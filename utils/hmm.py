...
class LinearGaussianHMM:
    ...
    def sample_state_sequence(self, sequence_length):
        ...
        for sample_nb in range(1, sequence_length):
            state_sequence[sample_nb] = MultivariateNormal(loc=self.model.transition.matrix @ state_sequence[sample_nb-1] + self.model.transition.offset, 
                                                            covariance_matrix=self.model.transition.cov).sample()
        
        return state_sequence

    def sample_joint_sequence(self, sequence_length):
        dim_x = self.model.emission.matrix.shape[0]
        ...
        for sample_nb in range(sequence_length):
            observation_sequence[sample_nb] = MultivariateNormal(loc=self.model.emission.matrix @ state_sequence[sample_nb] + self.model.emission.offset, 
