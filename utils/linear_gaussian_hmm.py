...
def sample_state_sequence(key, sequence_length, prior_params:PriorParams, transition_params:TransitionParams):
    ...
    state_sequence = state_sequence.at[0].set(multivariate_normal(key=key, mean=prior_params.mean, cov=prior_params.cov))

    for sample_nb in range(1, sequence_length):
        state_sequence = state_sequence.at[sample_nb].set(multivariate_normal(key=key, 
...
def sample_joint_sequence(key, sequence_length, model_params:ModelParams):
    ...
    state_sequence = sample_state_sequence(key, sequence_length, model_params.prior, model_params.transition)
    for sample_nb in range(sequence_length):
        observation_sequence = observation_sequence.at[sample_nb].set(multivariate_normal(key=key, 
