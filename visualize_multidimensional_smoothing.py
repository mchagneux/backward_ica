...
exp_dir = 'experiments/p_nonlinear/p_nonlinear_dim_2_with_help/trainings/nonlinear_johnson/2022_06_22__11_41_05'
...
#%% 
means_ffbsi, covs_ffbsi = jnp.mean(smoothing_paths, axis=1), jnp.var(smoothing_paths, axis=1)
key_phi, key_smooth_q = jax.random.split(key_phi, 2)

if isinstance(q, hmm.GeneralBackwardSmoother):
    means_q, covs_q = q.smooth_seq(key_smooth_q, obs_seq, phi, 1000)
else:     
...
for dim_nb in range(args.state_dim):
    ...
    utils.plot_relative_errors_1D(axes[dim_nb,0], state_seq[:,dim_nb], means_ffbsi[:,dim_nb], covs_ffbsi[:,dim_nb])


# state_seq = state_seq / jnp.linalg.norm(state_seq)
# means_q = means_q / jnp.linalg.norm

for dim_nb in range(args.state_dim):
    state_seq_dim = state_seq[:,dim_nb]
    means_q_dim = means_q[:,dim_nb]

    if isinstance(q, hmm.LinearBackwardSmoother):
        utils.plot_relative_errors_1D(axes[dim_nb,1], state_seq_dim, means_q_dim, covs_q[:,dim_nb,dim_nb])
    else:
        utils.plot_relative_errors_1D(axes[dim_nb,1], state_seq_dim, means_q_dim, covs_q[:,dim_nb])

plt.autoscale(True)
plt.tight_layout()
plt.savefig('smoothing_eval')
#%%s